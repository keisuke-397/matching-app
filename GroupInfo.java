...
public class GroupInfo implements Serializable{
	public int MAX=100;	//イイネ数をやグループ数の保持する限界数。変えてもいい
	public int state=0;		//グル作成なら0,プロフ更新なら1

	public UUID groupNumber;	//このグルの識別番号。クライアントがグルを作ったのち、サーバが割り振る
	public String name="グループ名";		//グループ名
	public String relation="友達";		//グルのメンバの関係性
	public UUID[] sendGood=new UUID[MAX];		//このグルがイイネを送った相手のグルの識別番号
	public UUID[] receiveGood=new UUID[MAX];		//このグルにイイネを送った相手のグルの識別番号
	public UUID[] matchedGroup=new UUID[MAX];		//このグルとマッチングした相手のグルの識別番号
	public transient BufferedImage mainPhoto;		//グルの写真
	public int hostUser=0;		//グルのホストユーザ(=グルを作った人)
	public int[] nonhostUser=new int[4];		//グルのホストでないユーザ。グルに招待されている時点で追加する
	public int purpose=0;			//グルの目的。Client.javaの配列Purposeで文字列に変える
	public String comment="仲良くしてね！";		//グルの一言コメント
	public int numberOfMember=1;	//グルの人数。グループから抜ける機能はなくなったので、招待した時点で固定。
	public boolean isGathered=false;	//グルが全員集まったならtrueに変換し、検索に引っかかるようになる。
